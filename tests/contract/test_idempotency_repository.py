...
def test_atomic_claim_processing_conflict_and_pre_start_reclaim() -> None:
    ...
    acquired = repository.claim(key, "a" * 64, trace1, FIXED_UTC)
    assert acquired.disposition.value == "acquired"
    processing = repository.claim(key, "a" * 64, trace2, FIXED_UTC)
    assert processing.disposition.value == "processing" and processing.original_trace_id == trace1
    assert repository.claim(key, "b" * 64, trace2, FIXED_UTC).disposition.value == "conflict"
    repository.mark_pre_start_failed(key, acquired.owner_token, "agent_unavailable", FIXED_UTC)
    reclaimed = repository.claim(key, "a" * 64, trace2, FIXED_UTC)
    assert reclaimed.disposition.value == "acquired" and reclaimed.attempt == 2
    with pytest.raises(ConditionalWriteFailed):
        repository.mark_running(key, reclaimed.owner_token, trace1, FIXED_UTC)
